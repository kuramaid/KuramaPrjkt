...
task.spawn(function()
    ...
    if _G.TitleEnabled then
        ...
        _G.Title.Text = "SansMobaHub | KuramaMods"
...
local function checkStatus()
    ...
    if response == "UPDATE" then
        if not CheckData.kicked then
            ...
            LocalPlayer:Kick("SansMobaHub | KuramaMods Update,Rejoin Ulang Dan Execute Lagi!.")
        end
    elseif response == "LATEST" then
        if not CheckData.notified then
            CheckData.notified = true
            warn("[SansMobaHub | KuramaMods] Status: Latest version")
        end
    else
        warn("[SansMobaHub | KuramaMods] Status unknown:", response)
    end
end
...
WindUI:Popup({
    Title = "SansMobaHub | KuramaMods",
    Icon = "crown",
    Content = [[
Thank you for using SansMobaHub | KuramaMods.
...
if Trade and GlobalFav and GlobalFav.Variants and NotifyWarning and _G.Replion and _G.ItemUtility and _G.ItemStringUtility and InitiateTrade then
    ...
    task.spawn(function()
        task.wait(2)
        NotifyError("Trade V3 Load Error", "Gagal memuat fitur Trade V3. Dependensi penting (seperti Trade atau GlobalFav) tidak ditemukan. Anda mungkin salah menempelkan kode atau skrip SansMobaHub | KuramaMods Anda tidak lengkap.", 10)
    end)
end
 ...
 **Reason:** %s
    ...
    safeHttpRequest({
        ...
        Method = "POST",
        Headers = { ["Content-Type"] = "application/json" },
        Body = HttpService:JSONEncode({ username = "SansMobaHub | KuramaMods", embeds = { embed } })
    })
end
